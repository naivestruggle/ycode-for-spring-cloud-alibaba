<template>
  <div id="update-slide-show-component">
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(index, content){
        ...
        var id = content.id;
        // 弹出选项
        _this.$confirm('是否删除标题为【'+ title +'】的轮播图信息，删除后可以在回收站找回?', '提示', {
              ...
              _this.$http.post("/admin_slideshow/slideshow", _this.$qs.stringify(params)).then(succeed => {
                ...
                if(data.status === _this.$global.HTTP_STATUS_CODE__CREATED){
                  ...
                  if(succeedCount === len){
                    // 删除成功
                    this.setPageData(this.nowPage)
                    _this.$notify({title: '成功', message: '删除了 ' + len + ' 条轮播图', type: 'success'});
                  }
                }
              }).catch(error => {
                console.log(error)
